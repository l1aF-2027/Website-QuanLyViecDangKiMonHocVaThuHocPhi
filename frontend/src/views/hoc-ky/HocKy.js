...
import HocKyDialog from './HocKyDialog';
import { deleteHocKy, getAllHocKy } from './HocKyServices';
...
const HocKy = () => {
  ...
  const handleOke = async () => {
    try {
      await deleteHocKy(item?.idHocKy);
  ...
  return (
    <PageContainer title="Học kỳ" description="">
      <DashboardCard title="Danh sách kỳ học">
        <MaterialTable
          title={
          ...
          columns={[
            ...
            { title: 'Học kỳ', field: 'tenHocKy' },
            { title: 'Năm học', field: 'namHoc' },
          ]}
          data={listItems}
      ...
      {open && (
        <HocKyDialog open={open} handleClose={handleClose} item={item} updateData={getAll} />
      )}
      {openDelete && (
