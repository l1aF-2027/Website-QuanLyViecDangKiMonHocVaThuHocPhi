...
import QueQuanDialog from './QueQuanDialog';
import { deleteQueQuan, getAllQueQuan } from './QueQuanServices';
...
const QueQuan = () => {
  ...
  const handleOke = async () => {
    try {
      await deleteQueQuan(item?.idQueQuan);
  ...
  const getAll = async () => {
    try {
      const data = await getAllQueQuan();
  ...
  return (
    <PageContainer title="Quê quán" description="">
      <DashboardCard title="Danh sách quê quán">
        <MaterialTable
          title={
          ...
          columns={[
            ...
            { title: 'Tên huyện', field: 'tenHuyen' },
            { title: 'Tên tỉnh', field: 'tenTinh' },
          ]}
          data={listItems}
      ...
      {open && (
        <QueQuanDialog open={open} handleClose={handleClose} item={item} updateData={getAll} />
      )}
      {openDelete && (
