...
import LopHocDialog from './LopHocDialog';
import { deleteLopHoc, getAllLopHoc } from './LopHocServices';
...
const LopHoc = () => {
  ...
  const handleOke = async () => {
    try {
      await deleteLopHoc(item?.idLopHoc);
  ...
  return (
    <PageContainer title="Lớp học" description="">
      <DashboardCard title="Danh sách lớp học">
        <MaterialTable
          title={
      ...
      {open && (
        <LopHocDialog open={open} handleClose={handleClose} item={item} updateData={getAll} />
      )}
      {openDelete && (
